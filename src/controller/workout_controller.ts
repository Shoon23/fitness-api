...
async function filter_workout(req: Request, res: Response) {
  ...
  try {
    ...
    // filtering
    const level_filter = workouts.filter((workout) => workout.level === level);
...
// add workout
async function assigning_workouts(req: Request, res: Response) {
  try {
  } catch (error) {
    console.log(error);
